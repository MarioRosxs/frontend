...
methods: {
  ...
  */
  ...
  // Asigna el ítem directamente
  this.editedItem = item;
  // Actualiza el índice solo si es un elemento existente
...
async deleteItemConfirm() {
  ...
  try {
    await this.axiosRequest(`https://localhost:3001/personas/${itemId}`, 'delete');
    ...
    async save () { //Crea y edita registros.
      try {
        if (this.editedIndex > -1) {
          ...
          const response = await this.axiosRequest(`https://localhost:3001/personas/${this.editedItem.id}`, 'put', this.editedItem);
          console.log('Elemento editado con exito');
          this.personas[this.editedIndex] = response;
        } else {
          // Insertar nuevo registro
          const response = await this.axiosRequest('https://localhost:3001/personas/', 'post', this.editedItem);
